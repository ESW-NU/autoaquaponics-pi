...
GPIO.gpio_claim_output(h, pin_num)
GPIO.gpio_claim_output(h, pin_num2)
...
def get_data(last_distance, last_wtemp, last_hum, last_atemp):  #main function that calls on all other functions to generate data list
    ...
    GPIO.gpio_write(h, pin_num, GPIO.HIGH)  #turn TDS sensor on
    sleep(0.5)
    ...
    if wtemp == np.nan:  #use last wtemp value if it's NaN
        ...
        TDS = get_tds(wtemp)
    GPIO.gpio_write(h, pin_num,GPIO.LOW)  #turn TDS sensor off
    sleep(0.5)
...
def get_tds(wtemp):
    ...
    Vc=0.98
    temp_calibrate= 25          #measured water temp when calibrating
...
def get_distance(last_distance):  #output distance in cm
    ...
    # set Trigger to HIGH
    StopTime = time.time()
    GPIO.gpio_write(h, GPIO_TRIGGER, True)

    # set Trigger after 0.01ms to LOW
    time.sleep(0.00006)
    GPIO.gpio_write(h, GPIO_TRIGGER, False)
    StartTime = time.time()
...
if __name__ == "__main__":
    print("Hello world")
    # print(get_dht())
    reading0 = chan.voltage
    reading1 = chan1.voltage
    reading2 = chan2.voltage
    print(f"{reading0=} {reading1=} {reading2=}")
    v = chan1.voltage
    print(v)
    tds = get_tds(22)
    print(f"get_tds returns: {tds}")
